...
public class OracleOptimizeTriggers {
	...
	static String[] sampleqrys = {
		//"CREATE OR REPLACE TRIGGER  Uuser_cameras1017121279 BEFORE UPDATE ON user_cameras FOR EACH ROW DECLARE DeleteIT_DLL_Val BINARY_INTEGER; KeyToDelete  CLOB; /* DeleteKey CLOB; */ BEGIN KeyToDelete := ' ';  KeyToDelete := CONCAT('_9',CONCAT(:OLD.IS_STREAMING, :OLD.USER_ID)); DeleteIT_DLL_Val := KOSARTriggerDeleteCall('RAYS', KeyToDelete, 0); KeyToDelete := CONCAT('_9',CONCAT(:NEW.IS_STREAMING, :NEW.USER_ID)); DeleteIT_DLL_Val := KOSARTriggerDeleteCall('RAYS', KeyToDelete, 0); END;",
		//"CREATE OR REPLACE TRIGGER  Uuser_cameras1011212977959 BEFORE UPDATE ON user_cameras FOR EACH ROW DECLARE DeleteIT_DLL_Val BINARY_INTEGER; KeyToDelete  CLOB; /* DeleteKey CLOB; */ BEGIN KeyToDelete := ' ';  KeyToDelete := CONCAT('_7959881212759',CONCAT(:OLD.CAMERA_ID, :OLD.USER_ID)); DeleteIT_DLL_Val := KOSARTriggerDeleteCall('RAYS', KeyToDelete, 0); KeyToDelete := CONCAT('_7959881212759',CONCAT(:NEW.CAMERA_ID, :NEW.USER_ID)); DeleteIT_DLL_Val := KOSARTriggerDeleteCall('RAYS', KeyToDelete, 0); END;",
		//"CREATE OR REPLACE TRIGGER  UUSER_CAMERAS2457841 BEFORE UPDATE ON USER_CAMERAS FOR EACH ROW DECLARE DELETEKEY CLOB; KEYTODELETE CLOB; DeleteIT_DLL_Val BINARY_INTEGER;  BEGIN  USER_CAMERAS2457841(:OLD.USER_ID, :OLD.IS_STREAMING /*, DeleteKey*/);  USER_CAMERAS2457841(:New.USER_ID, :New.IS_STREAMING /*, DeleteKey*/);  END; "
		"CREATE OR REPLACE TRIGGER  USERSI BEFORE INSERT ON USERS FOR EACH ROW DECLARE KEYTODELETE CLOB; DeleteIT_DLL_Val BINARY_INTEGER;  DELETEKEY CLOB; BEGIN  KEYTODELETE := CONCAT('USERS', KEYTODELETE); DELETEKEY := CONCAT(DELETEKEY, CONCAT(' ', KEYTODELETE)); IF (DELETEKEY != ' ') THEN  DeleteIT_DLL_Val := KOSARTriggerDeleteMultiCall('10.0.0.205:4000,10.0.0.205:4001,10.0.0.205:4002,10.0.0.205:4003,10.0.0.205:4004', 'RAYS', DELETEKEY, 0); IF (DeleteIT_DLL_Val != 0) THEN RAISE_APPLICATION_ERROR( -20001, 'Failed to connect to KOSAR KVS CORE.'); END IF; END IF; END; ",
		"CREATE OR REPLACE TRIGGER  UINC BEFORE INSERT ON USERS FOR EACH ROW begin select useridInc.nextval into :new.userid from dual;end;"
	};
	...
	public static String MergeTriggers(Vector<String> tz){
		...
		if (Merged.length() == 0){
			System.out.println("Error in OracleOptimizeTrigger.MergeTriggers: DECLARE keyword is missing.  All triggers must have this.");
			return null;
		}
		...
		for (int i=0; i < tz.size(); i++){
			...
			//Parse the procedure calls and eliminate duplicates using a hash map.
			String procedureBody = elt.substring(bodystarts, bodyends);
			...
			while (comp != true){
				...
				if (proccall.indexOf("IF ") >= 0){
					...
					if (procedureBody.substring(procstarts+ifstart,procstarts+ifends).contains( "KOSARTriggerDeleteMultiCall".toUpperCase() )){
						int endiflen = " END IF; ".length();
						ifends += endiflen;
						//ifends += procedureBody.substring(procstarts+ifends+endiflen).indexOf(" END IF; ");
					}
	...
	public static String OptimizeTrigger(String trig){
		...
		if (trig.indexOf("IF (DELETEKEY != ' ') THEN  DeleteIT_DLL_Val := KOSARTriggerDeleteMultiCall(") < 0)
			trig = trig.replaceAll("END;", "IF (DELETEKEY != ' ') THEN  DeleteIT_DLL_Val := KOSARTriggerDeleteMultiCall('"+QueryToTrigger.getIPport()+"', 'RAYS', DELETEKEY, 0); IF (DeleteIT_DLL_Val != 0) THEN RAISE_APPLICATION_ERROR( -20001, 'Failed to connect to KOSAR KVS CORE.'); END IF; END IF; END;");
