...
inline static void push( implicit_heap *queue, uint32_t src, uint32_t dst );
inline static void dump( implicit_heap *queue, implicit_node *node, uint32_t dst );
...
inline key_type pq_get_key( implicit_heap *queue, implicit_node *node )
{
    UNUSED( queue );
    return node->key;
}

inline item_type pq_get_item( implicit_heap *queue, implicit_node *node )
{
    UNUSED( queue );
    return node->item;
}

inline uint32_t pq_get_size( implicit_heap *queue )
{
    return queue->size;
}
...
inline implicit_node* pq_find_min( implicit_heap *queue )
{
    if ( pq_empty( queue ) )
        return NULL;
...
inline key_type pq_delete( implicit_heap *queue, implicit_node* node )
{
...
inline void pq_decrease_key( implicit_heap *queue, implicit_node *node,
    key_type new_key )
{
    node->key = new_key;
    heapify_up( queue, node );
...
inline void pq_increase_key( implicit_heap *queue, implicit_node *node,
    key_type new_key )
{
    node->key = new_key;
...
inline bool pq_empty( implicit_heap *queue )
{
    return ( queue->size == 0 );
}
